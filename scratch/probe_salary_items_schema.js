...
async function probe() {
  console.log('Probing salary_items schema...');
  const { data, error } = await supabase.from('salary_items').select('*').limit(1);
  if (error) {
  ...
  const columns = [
    'id', 'salary_month_id', 'employee_id', 'base_salary', 'net_salary',
    'notes', 'created_at', 'updated_at'
  ];

  for (const col of columns) {
    const { error: colErr } = await supabase.from('salary_items').select(col).limit(1);
